---
name: scrapecreators
description: Public social data through the ScrapeCreators API — profiles, posts, comments, transcripts, ad libraries, search, and trending content across TikTok, Instagram, YouTube, LinkedIn, Twitter/X, Reddit, Facebook, and 20+ more platforms. Use when the marketer asks about a creator, a competitor's social presence, what content is working, or what ads a brand is running; covers the creator_profile and creator_posts tools plus routing for the full 110-endpoint API.
---

> Folded from the official vendor skill: `scrapecreators-api` in
> [ScrapeCreators/social-media-research-skills](https://github.com/ScrapeCreators/social-media-research-skills)
> (the repo their docs point to: https://docs.scrapecreators.com/integrations/agent-skill/),
> fetched 2026-08-10. Licensed MIT per the repo's `.claude-plugin/plugin.json`.
> One path corrected against the API's OpenAPI spec
> (https://docs.scrapecreators.com/openapi.json): YouTube channel videos live
> at `/v1/youtube/channel-videos`, not `/v1/youtube/channel/videos` as the
> upstream routing table has it.
...
The montology MCP tools cover the first questions asked of any creator; the
full API above is there when they run out.

- `creator_profile(platform, handle)` first — confirms the handle resolves
  and gives the denominator (followers) every engagement read needs.
- `creator_posts(platform, handle)` — one page of recent posts with counts.
  Engagement RATE (interactions / followers) is the comparable number;
  raw likes are not comparable across creators.
- Public data only. If a profile is private or missing, report that as the
  finding — do not try alternate routes.
- Platforms name fields differently; normalise before comparing across
  platforms, and say which platform each number came from.
- Most endpoints cost 1 credit, but check the credit-cost table above before
  an expensive call (audience demographics is 26) — and batch your questions
  before calling.
