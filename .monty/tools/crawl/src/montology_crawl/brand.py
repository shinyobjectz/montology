...
def scaffold(brand: str, kit_json: str) -> str:
    ...
    return (f"scaffolded projects/{brand}/ — tokens.ts ({len(colors[:12])} colors, "
...
def lint(brand: str) -> list[str]:
    ...
    mf = root / "manifest.json"
    if not mf.exists():
        return [f"FAIL projects/{brand}: no manifest.json — scaffold first"]
    try:
        manifest = json.loads(mf.read_text())
    except json.JSONDecodeError as e:
        return [f"FAIL projects/{brand}/manifest.json does not parse: {e}"]

    if not manifest.get("source") or not manifest.get("derived"):
        report.append(f"FAIL projects/{brand}: manifest missing source/derived — provenance is not optional")
    if not (root / "tokens.ts").exists():
        report.append(f"FAIL projects/{brand}: no tokens.ts — components have nothing to import")

    built = candidates = 0
    for comp in manifest.get("components", []):
        f = root / comp.get("file", "")
        tag = f"projects/{brand}/{comp.get('file', '?')}"
